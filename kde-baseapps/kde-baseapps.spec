...
Requires: kdepasswd = %{version}-%{release}
...
Requires: libkonq%{?_isa} = %{?epoch:%{epoch}:}%{version}-%{release}
%description libs
%{summary}.
...
Requires: libkonq%{?_isa} = %{version}-%{release}
...
%check
appstream-util validate-relax --nonet %{buildroot}%{_datadir}/appdata/kfind.appdata.xml ||:
appstream-util validate-relax --nonet %{buildroot}%{_datadir}/appdata/konqueror.appdata.xml ||:
